...
can be queried and visualized in a manner that is both efficient and user-friendly. We will specifically demonstrate 
that trial data can be filtered based on metadata corresponding to experimental variables, rendered into basic data 
visualizations, accessed through NLP-generated SQL queries, and supported by an improved and intuitive user interface, 
all of which will be delivered through a simple Minimal Viable Product (MVP). Implementing these features on a small 
scale will provide evidence that our design will perform reliably and consistently at full scale, and most importantly, 
will establish a strong foundation for expanding to the full dataset while supporting robust natural language querying.
}
