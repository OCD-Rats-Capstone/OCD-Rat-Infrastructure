...
\midrule
Nov 13, 2025 & 1.0 & Initial Module Interface Specification (Rev 0)\\
Apr 6, 2026 & 2.0 & Final Revision: added cross-document references, M3/M4/M6 semantic corrections, mathematical formalization (M2), M9 access routine semantics\\
\bottomrule
\end{tabularx}
...
\textbf{InfraRat}, a data analysis web application designed to visualize, query, and process behavioural data from experiments involving rats with Obsessive-Compulsive Disorder (OCD). The system enables researchers to upload experimental trial data, perform natural language–based searches, and generate dynamic visualizations for behavioural comparisons and trend analysis.
...
This MIS should be read in conjunction with the following project documents:
\begin{itemize}
  ...
  \item \textbf{Module Guide (MG):} \url{https://github.com/OCD-Rats-Capstone/OCD-Rat-Infrastructure/blob/main/docs/Design/SoftArchitecture/MG.pdf}
...
\item exceptions: $(natural\_language$ IS NULL $=> empty\_input),$ \\
$(!is\_valid\_output(natural\_language) => incoherent\_prompt)$
\end{itemize}
...
\subsection{Formal Specification of NLP Query Processing}

This section provides a mathematical formalization of the NLP Query Processor Module (M2) using a finite state machine (FSM) and formal function specifications with set-theoretic notation.

\subsubsection{Finite State Machine}

The query processing pipeline is modeled as a deterministic finite state machine $\mathcal{M} = (S, \Sigma, \delta, s_0, F)$ where:

\begin{itemize}
  \item $S = \{\text{IDLE}, \text{VALIDATING\_INPUT}, \text{CALLING\_LLM},$ \\
        $\text{VALIDATING\_OUTPUT}, \text{COMPLETE}, \text{ERROR}\}$
  \item $\Sigma = \{\text{receive\_query}, \text{input\_valid}, \text{input\_invalid}, \text{llm\_success},$ \\
        $\text{llm\_failure}, \text{output\_valid}, \text{output\_invalid}, \text{reset}\}$
  \item $s_0 = \text{IDLE}$
  \item $F = \{\text{COMPLETE}\}$
\end{itemize}

The transition function $\delta : S \times \Sigma \rightarrow S$ is defined by the following table:

\begin{center}
\begin{tabular}{l l l}
\toprule
\textbf{Current State} & \textbf{Input Symbol} & \textbf{Next State} \\
\midrule
IDLE & receive\_query & VALIDATING\_INPUT \\
VALIDATING\_INPUT & input\_valid & CALLING\_LLM \\
VALIDATING\_INPUT & input\_invalid & ERROR \\
CALLING\_LLM & llm\_success & VALIDATING\_OUTPUT \\
CALLING\_LLM & llm\_failure & ERROR \\
VALIDATING\_OUTPUT & output\_valid & COMPLETE \\
VALIDATING\_OUTPUT & output\_invalid & ERROR \\
ERROR & reset & IDLE \\
COMPLETE & reset & IDLE \\
\bottomrule
\end{tabular}
...
All undefined transitions remain in the current state: $\forall (s, \sigma) \notin \text{dom}(\delta), \; \delta(s, \sigma) = s$.

\subsubsection{Formal Function Specifications}

The core transformation performed by the NLP Processor is formalized as follows.

\paragraph{Domain Definitions.}

Let $\mathcal{L}$ be the set of all non-empty strings (natural language inputs), and let $\mathcal{Q}$ be the set of all well-formed structured query strings. We define the output grammar as:

\[
\mathcal{Q} = \{ q \mid q = \langle a_1, o_1, v_1 \rangle ; \ldots ; \langle a_n, o_n, v_n \rangle \}
\]

where $a_i \in \textit{Schema\_Attributes}$, $o_i \in \textit{Valid\_Operators}$, and $v_i$ is a literal value compatible with the attribute type of $a_i$.

\paragraph{NLP\_Processor Function.}

\[
\text{NLP\_Processor} : \mathcal{L} \rightarrow \mathcal{Q}
\]

\noindent \textbf{Precondition:}
\[
\text{pre}(nl) \equiv nl \neq \epsilon \land |nl| > 0
\]

\noindent \textbf{Postcondition:}
\[
\text{post}(nl, q) \equiv q \in \mathcal{Q} \land \text{is\_valid\_output}(q) = \text{True}
\]

\noindent \textbf{Exception:}
\begin{align*}
\text{exc}(nl) \equiv &(nl = \epsilon \Rightarrow \text{empty\_input}) \lor \\
                      &(\text{is\_valid\_output}(\text{LLM\_API\_Call}(nl)) = \text{False} \Rightarrow \text{incoherent\_prompt})
\end{align*}

\paragraph{Validation Predicate.}

The output validation predicate $\text{is\_valid\_output} : \text{String} \rightarrow \mathbb{B}$ is defined as:

\begin{align*}
\text{is\_valid\_output}(r) \equiv \forall \langle a_i, o_i, v_i \rangle \in \text{parse}(r) : \;&a_i \in \textit{Schema\_Attributes} \land \\
                                                                                              &o_i \in \textit{Valid\_Operators}
\end{align*}

where $\text{parse} : \text{String} \rightarrow \mathcal{P}(\textit{Schema\_Attributes} \times \textit{Valid\_Operators} \times \text{Value})$ decomposes the LLM response into a set of attribute-operator-value triples.

\paragraph{Composition.}

The full pipeline composition is defined by first letting:
\[ \text{LLM}(nl) = \text{LLM\_API\_Call}(nl, \textit{System\_Prompt}) \]
Then:

\[
\text{NLP\_Processor}(nl) = \begin{cases}
  \text{LLM}(nl) & \text{if } \text{is\_valid\_output}(\text{LLM}(nl)) \\
  \bot_{\text{incoherent\_prompt}} & \text{otherwise}
\end{cases}
\]

where $\bot_{\text{incoherent\_prompt}}$ denotes exceptional termination with the \texttt{incoherent\_prompt} error.
...
\noindent $Submit\_Search\_Query(QueryParameters) \rightarrow DisplayedResults$:
\begin{itemize}
\item transition: $DataSet := HTTP\_Send\_Request(QueryParameters).body$
\item output: out $:= $ rendered view of $DataSet$ in the UI result panel
\item exception: $exc := (QueryParameters~\text{IS NULL} \Rightarrow invalid\_query)$
\end{itemize}

\noindent $Apply\_Filter(FilterCriteria) \rightarrow UpdatedResults$:
\begin{itemize}
\item transition: $DataSet := HTTP\_Send\_Request(FilterCriteria).body$
\item output: out $:= $ filtered subset of $DataSet$ rendered in the UI
\item exception: $exc := (FilterCriteria~\text{contains invalid attribute} \Rightarrow invalid\_filter)$
\end{itemize}

\noindent $Render\_Visualization(VisualizationConfig, DataSet) \rightarrow RenderedChart$:
\begin{itemize}
\item transition: None
\item output: out $:= $ SVG or canvas element generated by the Data Visualization Module (M6)
\item exception: $exc := (VisualizationConfig.type \notin \{\text{VTYPE\_BAR}, \text{VTYPE\_PLOT},$ \\
$\text{VTYPE\_PIE}, \text{VTYPE\_HEATMAP}, \text{VTYPE\_TIMESERIES}\} \Rightarrow unsupported\_chart\_type)$
\end{itemize}

\noindent $Download\_Data(FormatType, DataSet) \rightarrow File$:
\begin{itemize}
\item transition: None
\item output: out $:= $ file download initiated in the specified $FormatType$ (CSV, JSON)
\item exception: $exc := (FormatType \notin \{\text{CSV}, \text{JSON}\} \Rightarrow unsupported\_format)$ \\
$(DataSet~\text{IS NULL} \Rightarrow empty\_dataset)$
\end{itemize}
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
Process\_Structured\_Query & StructuredQuery & JSON Object & malformed query, empty result \\
Process\_NLP\_Query &  NaturalLanguageQuery & JSON Object & unrecognized intent, NLP failure \\
Get\_Dataset & QueryResultID & DataSet (JSON) & invalid ID, empty dataset \\
Request\_RawFiles & DataSet\{"FileLocations"\} & File[] & bad URL, missing files \\
Authenticate\_Request & AuthToken & AccessDecision & invalid token \\
\hline
\end{tabularx}
\end{center}
...
\noindent $Process\_Structured\_Query(StructuredQuery) \rightarrow JSON~Object$:
\begin{itemize}
\item transition: $RequestedQuery := Structured\_Query(StructuredQuery)$; $DataSet := Request\_DataSet(RequestedQuery, HostedDatabaseEndPoint)$
\item output: out $:=$ DataSet (JSON Object containing matching experimental session records)
\item exception: $exc := (StructuredQuery~\text{IS NULL} \Rightarrow malformed\_query)$ \\
$(DataSet~\text{IS NULL} \Rightarrow empty\_result)$
\end{itemize}

\noindent $Process\_NLP\_Query(NaturalLanguageQuery) \rightarrow JSON~Object$:
\begin{itemize}
\item transition: $RequestedQuery := NLP\_Query(NaturalLanguageQuery)$; $DataSet := Request\_DataSet(RequestedQuery, HostedDatabaseEndPoint)$
\item output: out $:=$ DataSet (JSON Object containing matching experimental session records)
\item exception: $exc := (NLP\_Query~\text{fails validation} \Rightarrow unrecognized\_intent)$ \\
$(NLPModule~\text{unavailable} \Rightarrow NLP\_failure)$
\end{itemize}

\noindent $Authenticate\_Request(AuthToken) \rightarrow AccessDecision$:
\begin{itemize}
\item transition: None
\item output: out $:=$ access decision from the Authentication and Access Control Module (M8)
\item exception: $exc := (AuthToken~\text{IS NULL} \lor \text{expired} \Rightarrow invalid\_token)$
\end{itemize}
...
\begin{itemize}
  \item transition: $DataSet := $ \\
  if $(QueryType == \text{'NaturalLanguage'}) \{NLP\_Query(HTTP\_Request)\}$ \\
  elseif $(QueryType == \text{'Structured'}) \{Structured\_Query(HTTP\_Request)\}$
  \item exception: $exc := (QueryType \notin \{\text{'NaturalLanguage'}, \text{'Structured'}\} \Rightarrow Bad\_Query\_Type)$
\end{itemize}
...
While individual visualization render calls are functionally pure (all required data is provided via parameters), the module maintains configuration state to track the active chart settings and supported visualization types.

\begin{itemize}
  \item \textbf{ActiveChartConfig}: dict -- stores the current chart type, axis labels, color scheme, and display options for the active visualization session.
  \item \textbf{SupportedVTypes}: set of String -- the set of currently supported visualization types, initialized from the exported constants (VTYPE\_BAR, VTYPE\_PLOT, VTYPE\_PIE, VTYPE\_HEATMAP, VTYPE\_TIMESERIES). This state allows the module to be extended with new chart types as anticipated by AC4 in the Module Guide.
\end{itemize}
...
\section{MIS of M8: Authentication and Access Control Module (Descoped)} \label{Module 8}

\textit{*Note: This module was part of the original design but was descoped from the final implementation due to time constraints. The specification below is retained for architectural completeness.*}
...
\noindent \texttt{logEvent(message, level)}:
\begin{itemize}
\item transition: $errorBuffer := errorBuffer \cup \{(\text{message}, \text{level}, \text{timestamp})\}$
\item output: out $:=$ confirmation string indicating successful log entry
\item exception: $exc := (\text{level} \notin \text{LOG\_LEVELS} \Rightarrow ValidationError)$
\end{itemize}

\noindent \texttt{isTrue(a, b)}:
\begin{itemize}
\item transition: None
\item output: out $:= (a = b)$
\item exception: $exc := (\text{type}(a) \neq \text{type}(b) \Rightarrow ValidationError)$
\end{itemize}

\noindent \texttt{getFaultHistory(moduleID, timeRange)}:
\begin{itemize}
\item transition: None
\item output: out $:= \{e \in \text{persistentLog} \mid e.\text{module} = \text{moduleID} \land e.\text{time} \in \text{timeRange}\}$
\item exception: $exc := (\text{persistentLog unavailable} \Rightarrow DatabaseError)$
\end{itemize}

\noindent \texttt{raiseError(message, severity)}:
\begin{itemize}
\item transition: $errorBuffer := errorBuffer \cup \{(\text{message}, \text{severity}, \text{timestamp})\}$; triggers alert if $\text{severity} \geq \text{ERROR}$
\item output: out $:= True$ if error was successfully raised and recorded
\item exception: $exc := (\text{log write fails} \Rightarrow IOError)$
\end{itemize}
...
\section*{Appendix --- Module Implementation Reflection}

\begin{enumerate}
  \item What did you learn by implementing another team’s module?
  
  A major takeaway from this experience was being able to interpret the other team’s module interface specifications 
  and turn that into an implementation specifically developed into the other team’s existing technology stack. 
  Since the module interface specification gives you a generalized specification, we learned how to take this 
  generalized format and develop its internally and externally offered services in a specific technology which was React. 
  Besides the implementation, another learning outcome was learning how difficult it is to set up a project 
  where you have no development experience in, and how to properly debug errors in the setup process. 
  The other team’s documentation was well developed but our team experienced challenges during setup due to 
  the configurations of our personal machines, with extra difficulty introduced when trying to set up 
  the other team’s project on machines with different operating systems.

  \item Were all the details you needed in the documentation, or did you need to make assumptions, or ask the other team questions?
  
  For the most part the module interface specification was well documented, but some assumptions needed to be made, 
  specifically about the exported constants. The other team defined two constants pertaining to sets of different graphs, 
  and file formats for exporting. The sets of export graphs and export file formats were only abstractly defined, 
  meaning there was no set size given or no actual set elements defined (i.e.: set(GraphType) = {Barplot, Venn Diagram}). 
  As such, our team had to make assumptions about what exportable graph types and file formats we implemented 
  according to the nature of the data we were presented with.

  \item If your team also had another team implement one of your modules, what was this experience like?
  
  From what we observed from another team’s implementation of our Query Processor Module, 
  it was clear that our MIS for that module was missing multiple points of crucial information in terms of 
  implementation of internal components and outputs to our local functions. Although our 
  exported access programs allowed a complete module implementation, we did not provide all the required information 
  for the local functions such that they can support the functionality of the exported access programs which is 
  directly tied to the essential functionality of the module being implemented. Other than some gaps in information, 
  the implementing team did not reach out with any questions or clarifications while at the same time completing a 
  somewhat implementation of our module. One interesting thing that we observed over an extended development time 
  is how an initial module interface specification may no longer be viable according to the current design, 
  which is the case with the current state of our project and the other team’s module implementation.

  \item Are there things in your documentation you could have changed to make the process go more smoothly for when an “outsider” completes some of the implementation?
  
  According to the feedback given by the team implementing our Query Processing Module, 
  our team should have included additional implementation details on how the LLM API is called. 
  In hindsight, additional information about this could have easily been included in the assumptions section 
  about what LLM would be used, what the API contract would be defined as such that you can implement the call 
  to the LLM’s API.
\end{enumerate}
