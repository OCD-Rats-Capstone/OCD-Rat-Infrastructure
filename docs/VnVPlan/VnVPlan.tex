...
April 1 2026 & 1.1 & Fixed typos and formatting issues\\
\bottomrule
\end{tabularx}
...
The project would be advanced because it uses many complex software engineering ideas and principles. Creating a visually appealing UI, integrating an NLP model, implementing the database inventory display while
also ensuring that the performance is excellent. These features will require a very in depth approach during the design and verification stages of the system.\\
...
\begin{itemize}
    ...
    \item \textbf{Initial State:} System deployed with an inventory of all available rat trial sessions (e.g., `61474', `61475', and so on...).
    \item \textbf{Input:} User opens the `ToolBox' interface. User then presses "Load Session" after selecting a session number.
    \item \textbf{Output:} All rat trial sessions appear for selection.
    \item \textbf{Test Case Derivation:} Requirement specifies all rat trial sessions must be available to to browse.
    \item \textbf{How test will be performed:} Manually navigate to the `ToolBox' page and confirm all available rat trial sessions are listed and accessible.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Control:} Manual (UI rendering check)
    \item \textbf{Initial State:} User has filtered dataset (non-empty result).
    \item \textbf{Input:} User types and submits a natural language query to the NLP model in "Select" mode.
    \item \textbf{Output:} Visualization component renders the converted SQL query from the user's natural language query, and displays tabulated SQL query results.
    \item \textbf{Test Case Derivation:} The fit criterion specifies at least one visual per result set; therefore, successful rendering of both the query results and SQL query fulfills this requirement at minimum.
    \item \textbf{How test will be performed:} Manually submit a natural language query (that is valid and applicable to the database) and confirm that the system generates a visual representation of the results and displays the corresponding SQL query.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Input Base Case:} user inputs question or phrase such as 'hello'
    \item \textbf{Output Base Case:} We expect the response to contain the token "Hello"
    \item \textbf{Initial State Edge Case 1:} API connection raises an exception and cannot complete requests.
    \item \textbf{Input Edge Case 1:} user inputs question or phrase such as 'hello'
    \item \textbf{Output Base Case 1:} User is informed of failure to call on model response
    \item \textbf{Test Case Derivation:} Our query module has a "query" section and an "ask" section. The ask section allows the user to interact with the model and learn more about the tool.
...
\begin{itemize}
    ...
    \item \textbf{Input Pattern:}
      \begin{itemize}
        \item Navigate each of available tabs in the NavBar (Home, Query, Inventory, Visualizations, About)
        ...
        \item Select each of the three available visualizations and select any combination of X and Y parameters. Ensure visualizations render in a user-friendly way
      \end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Input Case 3:} Inventory module backend endpoint, filter set in request body
    \item \textbf{Output Case 3:} 200 HTTP code, JSON response
    \item \textbf{Input Case 4:} Visualization module backend endpoint, visualization parameters in 
